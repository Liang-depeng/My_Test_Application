...
class HandlerWelcomeActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private fun initData() {
        image.setImageResource(R.mipmap.lover)
        ...
        override fun handleMessage(msg: Message?) {
            ...
            when (msg?.what) {
                TAG -> {
                    ...
                    if (loadingTime < 0) {
                        startActivity(Intent(this@HandlerWelcomeActivity, CaiPuActivity::class.java))
                        finish()
                    } else {
    ...
    object MyHandler : Handler() {
        ...
        override fun handleMessage(msg: Message?) {
            when (msg?.what) {
                0 -> {
                    Toast.makeText(MyApplication.getAppContent(), "hello", Toast.LENGTH_SHORT).show()
                }
                else -> {
